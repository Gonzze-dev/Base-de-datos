...
--EJECUTAR LOS SCRIPTS UNO POR UNO, DE FORMA SECUENCIAL
...
SELECT  mailExists('gonzaalo.erradonea@gmail.com');
...
--TEST 17 AGREGAR UN LIBRO CON ISBN YA REGISTRADO
...
--TEST 18 AGREGAR STOCK A UN LIBRO EXISTENTE
SELECT agregarLibroComprado('234-543-251', 10);

--TEST 19 AGREGAR STOCK A UN LIBRO NO EXISTENTE
SELECT agregarLibroComprado('1', 10);

--TEST 20 CONSULTAR LOS FAVORITOS DE UN USUARIO POR CUIL EXISTENTE
SELECT * FROM getFavUsuarioByCuil('20424644309');

--TEST 21 CONSULTAR LOS FAVORITOS DE UN USUARIO POR CUIL INEXISTENTE
SELECT * FROM getFavUsuarioByCuil('2009');

--TEST 22 CONSULTAR EL CARRITO DE UN USUARIO POR CUIL EXISTENTE
SELECT * FROM getCarritoUsuarioByCuil('20424644309');

--TEST 23 CONSULTAR EL CARRITO DE UN USUARIO POR CUIL INEXISTENTE
SELECT * FROM getCarritoUsuarioByCuil('2009');

--TEST 24 CONSULTAR FACTURAS DE UN USUARIO POR CUIL EXISTENTE
SELECT * FROM getFacturaUsuarioByCuil('20424644309');

--TEST 25 CONSULTAR FACTURAS DE UN USUARIO POR CUIL INEXISTENTE
SELECT * FROM getFacturaUsuarioByCuil('2049');

--TEST 26 OBTENER LIBROS VIA ISBN EXISTENTE
SELECT * FROM getLibroByIsbn('234-543-251');

--TEST 27 OBTENER LIBROS VIA ISBN NO EXISTENTE
SELECT * FROM getLibroByIsbn('123-23');

--TESTE 28 OBTENER LIBROS DE MAYOR A MENOR
SELECT * FROM getLibrosOrdenadosMayorAMenor();

--TESTE 29 OBTENER LIBROS DE MENOR A MAYOR
SELECT * FROM getLibrosOrdenadosMenorAMayor();

--TEST 30 OBTENER LIBROS MEDIANTE RANGO DE PRECIO
SELECT * FROM getLibrosEnRangoPrecio(100.00, 200.00);

--TEST 31 OBTENER LIBROS MEDIANTE RANGO DE PRECIO MAL PUESTO
SELECT * FROM getLibrosEnRangoPrecio(200.00, 100.00);

--TEST 32 OBTENER LIBROS POR TEMA EXISTENTE
SELECT  * FROM getLibrosByTema('Drama');

--TEST 33 OBTENER LIBROS POR TEMA INEXISTENTE
SELECT  * FROM getLibrosByTema('Dama');

--TEST 34 OBTENER LIBROS POR PALABRAS
SELECT * FROM getLibrosByTitulo('Harry');

--TEST 35 OBTENER LIBROS QUE NO COINCIDAN CON PALABRAS PUESTAS EN LA FUNCION
SELECT * FROM getLibrosByTitulo('Poer');

--TEST 36 COMPLETAR LA COMPRA DE UN USUARIO CON CARRITO EXISTENTE Y DIRECCION A DONDE ENVIAR LOS LIBROS
SELECT compraUsuarioRealizada(1);

--TEST 37 COMPLETAR LA COMPRA DE UN USUARIO CON CARRITO INEXISTENTE
SELECT compraUsuarioRealizada(0);

--TEST 38 COMPLETAR LA COMPRA DEL USUARIO SIN QUE ESTE TENGA UNA DIRECCION A DONDE ENVIAR LOS LIBROS COMPRADOS
SELECT compraUsuarioRealizada(3);

--TEST 39 COMPROBAR SI QUEDA STOCK CON UN NUMERO INFERIOR AL STOCK QUE HAY EN DICHO LIRBO
SELECT quedaStock(9, '234-543-251');

--TEST 40 COMPROBAR SI QUEDA STOCK CON UN NUMERO SUPERIOR AL STOCK QUE HAY EN DICHO LIRBO
SELECT quedaStock(11, '234-543-251');

--TESTE 41 COMPROBAR SI EXISTE UNA DIRECCION VIA ID DEL CARRITO DE UN USUARIO
SELECT existeDireccion(1);

--TESTE 42 COMPROBAR SI EXISTE UNA DIRECCION VIA ID DEL CARRITO DE UN USUARIO EL CUAL NO TIENE DIRECCION CARGADA
